...
③  restore the wallet  →  Bitcoin ₿`,
	...
	"index.textmap.p": "Hide the wallet passphrase in an ordinary text: a map of words for the database, and the text as a PDF to print.",

	"tm.title":        "Passphrase in a text",
	"tm.intro":        "Paste an ordinary text, such as a newspaper article, and type the wallet passphrase. Every character of the passphrase becomes the first letter or digit of one word, and you get a <strong>map</strong> of where those words are, plus a <strong>PDF</strong> of the text to print. The text alone gives nothing away, and neither does the map.",
	"tm.lang":         "Language of the map",
	"tm.headline":     "Headline (optional, it does not count)",
	"tm.text":         "Text",
	"tm.text.p":       "Separate paragraphs with an empty line; a text with no empty lines gets one paragraph per line. Choose one with plenty of numbers, so that every digit starts some word.",
	"tm.passphrase":   "Wallet passphrase",
	"tm.passphrase.p": "Lowercase letters a–z and digits only. It is not stored, and no page shows it again.",
	...
	"tm.res.paras.p":  "Check that these are the paragraphs you meant. The PDF has exactly these.",
	"tm.res.new":      "new map",

	"tm.map.head":      "Passphrase: put it together from the printed text",
	"tm.map.line":      "character %s:  paragraph %s, word %s",
	...
	"tm.rule.initial":  "from each word take its first letter or digit, in lowercase and without accents, and write the characters together without spaces",

	"tm.err.empty":    "Paste a text and type the passphrase.",
	"tm.err.chars":    "The passphrase can only contain lowercase letters a–z and digits, and these characters cannot be written that way: %s",
	"tm.err.missing":  "The text has no word starting with: %s. Add words or numbers that do, or choose another text.",
