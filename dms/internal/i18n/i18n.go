// Package i18n holds every string the switch sends to a person.
//
...
//
// Most of what the switch sends is plain-text e-mail, so S is the usual entry
// point; T exists for the two HTML pages (check-in and confirmation) and escapes
// its arguments, since a confirmer's name reaches them that way.
package i18n
...
// Lang is a language tag as used in the config, per recipient.
type Lang string
